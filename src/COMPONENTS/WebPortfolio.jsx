...
export default function WebPortfolio({ onPortfolioClick }) {
  return (
    <section className="max-w-full gap-10 overflow-hidden relative">
      ...
      >
        {myPortfolio
          ...
          .map((work) => (
            <SwiperSlide
              key={work.id}
              onClick={() => onPortfolioClick(work.id)} // Pass the ID to the parent
            >
              <div
      ...
      {/* Navigation arrows */}
      <div className="swiper-button-next absolute top-1/2 right-0 transform -translate-y-1/2 bg-white/70 backdrop-blur-md text-black rounded-full lg:h-12 lg:w-12 h-10 w-10 flex items-center justify-center cursor-pointer z-[9]">
        <FaArrowRightLong className="text-main text-xl" />
      </div>
      <div className="swiper-button-prev absolute top-1/2 left-0 transform -translate-y-1/2 bg-white/70 backdrop-blur-md text-black rounded-full lg:h-12 lg:w-12 h-10 w-10 flex items-center justify-center cursor-pointer z-[9]">
